...
<template>
    <v-card flat>
        <v-card-tit class="mt-5 text-h3 font-weight-bold text-center">{{ propiedad?.titulo }}</v-card-tit>
        <v-img :src="propiedad?.imagen" height="550" cover></v-img>
        <div class="bg-blue-grey-lighten-5 d-flex align-center flex-column flex-md-row">
            <v-card-text>
                Precio:
                <span class="font-weight-bold">{{ propiedadPrecio(propiedad?.precio) }}</span>
            </v-card-text>
            <v-card-text>
                Baños:
