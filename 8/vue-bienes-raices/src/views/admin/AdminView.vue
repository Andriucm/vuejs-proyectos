            ...
            <v-list-item v-for="propiedad in propiedadesCollection" :key="propiedad.id">
                <template v-slot:prepend>
                    <v-list-item-media :start="true">
                        <img :src="propiedad.imagen" :alt="'imagen de ' + propiedad.titulo" width="180">
                    </v-list-item-media>
                </template>
                ...
                <template v-slot:append>
                    <v-btn :to="{ name: 'editar-propiedad', params: { id: propiedad.id } }" color="info" flat
                        class="mr-2">Editar</v-btn>
                    <v-btn @click="deleteItem(propiedad.id, propiedad.imagen)" color="red-darken-3" flat>Eliminar</v-btn>
                </template>
            </v-list-item>
        </v-list>
    </v-card>
